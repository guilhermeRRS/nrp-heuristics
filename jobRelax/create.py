...
for time in [1800, 43200]:
	...
	for fast in [0, 1]:
		for factibilize in [0, 1]:
			if (fast == 1 and factibilize == 1) or (fast == 0 and factibilize == 0):
					...
					for dPartition in ["ALL", "UNITARY", "COUPLE", "QUARTER", "HALF"]:
						if not (iPartition == "ALL" and dPartition == "ALL"):
							for i in range(1,25):
								...
								if(mayRun):
									instance = str(i)
									fileName = f"{instance}_{description}_{iPartition}_{dPartition}_{fast}_{factibilize}.job"
									content = f"#PBS -N {instance}_{description}_{iPartition}_{dPartition}_{fast}_{factibilize}\n#PBS -l select=1:ncpus=4:nodetype=n40\n#PBS -l walltime=240:00:00\n\nmodule load python/3.6.8-gurobi\nmodule load gurobi/9.0.1\npython relax.py "
									content += f"{instance} {time} {description} {iPartition} {dPartition} {fast} {factibilize}"
									try:
