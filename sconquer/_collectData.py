...
def work_horizon(self, block):
	return int(block[0])
		
def work_shift(self, block):
	shifts = []
	durations = []
	r_t = []
	for line in block:
...
def work_staff(self, block, shifts, horizon):
	nurses = []
	m_max = []
	b_max = []
	b_min = []
...
def work_daysOff(self, block, nurses):
	n_i = []
	for nurse in nurses:
		n_i.append([])
...
def work_shiftOn_or_OffRequests(self, block, nurses, horizon, shifts):
	matrix = []
	for nurse in nurses:
		matrix.append([])
...
def work_cover(self, block, horizon, shifts):
	demands = []
	w_min = []
	w_max = []
	for i in range(horizon):
...
def get_data(self, path):
	...
	sucess, errors, fileConteds = self.readFile(path)

	if(sucess):
		s_horizon, s_shift, s_staff, s_daysOff, s_shiftOnRequests, s_shiftOffRequests, s_cover = self.splitFile(fileConteds)
		
		allData = {}
		
		allData["horizon"] = self.work_horizon(s_horizon)
		allData["shifts"], allData["durations"], allData["r_t"] = self.work_shift(s_shift)
		allData["nurses"], allData["m_max"], allData["b_max"], allData["b_min"], allData["c_max"], allData["c_min"], allData["o_min"], allData["a_max"] = self.work_staff(s_staff, allData["shifts"], allData["horizon"])
		allData["n_i"] = self.work_daysOff(s_daysOff, allData["nurses"])
		allData["q"] = self.work_shiftOn_or_OffRequests(s_shiftOnRequests, allData["nurses"], allData["horizon"], allData["shifts"])
		allData["p"] = self.work_shiftOn_or_OffRequests(s_shiftOffRequests, allData["nurses"], allData["horizon"], allData["shifts"])
		allData["demands"], allData["w_min"], allData["w_max"] = self.work_cover(s_cover, allData["horizon"], allData["shifts"])
		
		sucess, sets, parameters = self.convertFormat(allData)
		
		return sucess, [], sets, parameters
		
	return False, errors, None, None
	
def convertFormat(self, data):
