...
def create_shift_model(self, i, sets, parameters):
	...
	for c in range(1, parameters["c_min"][i]):
		for d in range(len(sets["D"]) - c - 1):
			j = d + 1
			shift_model.addConstr(gp.quicksum(sm_x[d][k] for k in range(len(sets["T"]))) + c - 1 - gp.quicksum(sm_x[m][n] for m in range(j, d + c +1) for n in range(len(sets["T"]))) + gp.quicksum(sm_x[d+c+1][o] for o in range(len(sets["T"]))) >= 0)
				
	#HC8
	for b in range(1, parameters["o_min"][i]):
		for d in range(len(sets["D"]) - b - 1):
			j = d + 1
			shift_model.addConstr(1 - gp.quicksum(sm_x[d][k] for k in range(len(sets["T"]))) + gp.quicksum(sm_x[m][n] for m in range(j, d + b +1) for n in range(len(sets["T"]))) - gp.quicksum(sm_x[d+b+1][o] for o in range(len(sets["T"]))) >= 0)
	shift_model.update()
