...
def fixVars(self, solution, x, Is, Dc, Ds):
	for i in Is:
		for d in range(Dc, Dc + Ds):
			for t in range(len(self.sets["T"])):
...
def get_extremeShifts(self, l_t):
	...
	self.shortestShiftSize = shortestShiftSize
	self.longestShiftSize = longestShiftSize
